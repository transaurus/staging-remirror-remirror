...
var import_querystringify = require("querystringify");
...
var ResizableIframeView = class extends import_prosemirror_resizable_view.ResizableNodeView {
  constructor(node, view, getPos, iframeOptions) {
    ...
    super({
      ...
      view,
      getPos,
      aspectRatio: import_prosemirror_resizable_view.ResizableRatioType.Flexible,
...
_IframeExtension_decorators = [(0, import_core.extension)({
  defaultOptions: {
    defaultSource: "",
...
var IframeExtension = class extends (_a = import_core.NodeExtension, _addIframe_dec = [(0, import_core.command)()], _addYouTubeVideo_dec = [(0, import_core.command)()], _updateIframeSource_dec = [(0, import_core.command)()], _updateYouTubeVideo_dec = [(0, import_core.command)()], _a) {
  constructor() {
    super(...arguments);
      ...
      toDOM: (node) => {
        ...
        return [
          ...
          __spreadProps(__spreadValues(__spreadValues({}, extra.dom(node)), rest), {
            class: (0, import_core.cx)(className, "".concat(className, "-").concat(type)),
            src,
  ...
  updateIframeSource(src) {
    return ({ tr, dispatch }) => {
      const iframeNode = (0, import_core.findSelectedNodeOfType)({ selection: tr.selection, types: this.type });
      if (!iframeNode) {
        return false;
      }
...
function parseUrl(url) {
  ...
  parser.href = url;
  const searchObject = (0, import_querystringify.parse)(parser.search);
  return {
...
function createYouTubeUrl(props) {
  ...
  const searchObject = (0, import_core.object)();
  if (!showControls) {
  ...
  return "".concat(urlStart, "/embed/").concat(id, "?").concat((0, import_querystringify.stringify)(searchObject));
}
