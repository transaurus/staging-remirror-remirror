...
_YjsExtension_decorators = [extension({
  defaultOptions: {
    getProvider: () => {
      invariant(false, {
...
var YjsExtension = class extends (_a = PlainExtension, _yUndo_dec = [command({
...
})], _undoShortcut_dec = [keyBinding({ shortcut: NamedShortcut.Undo, command: "yUndo" })], _redoShortcut_dec = [keyBinding({ shortcut: NamedShortcut.Redo, command: "yRedo" })], _a) {
  constructor() {
    super(...arguments);
  ...
  createExternalPlugins() {
    ...
    const plugins = [
      ySyncPlugin(type, __spreadProps(__spreadValues({}, syncPluginOptions), { mapping })),
      yCursorPlugin(
  ...
  yUndo() {
    return nonChainable((props) => {
      ...
      const undoManager = (_a2 = yUndoPluginKey.getState(state)) == null ? void 0 : _a2.undoManager;
