...
function pasteRules(pasteRules2) {
  const sortedPasteRules = (0, import_core_helpers.sort)(
    pasteRules2,
    (a, z) => {
      var _a, _b;
      return ((_a = z.priority) != null ? _a : import_core_constants.ExtensionPriority.Low) - ((_b = a.priority) != null ? _b : import_core_constants.ExtensionPriority.Low);
    }
  );
    ...
    props: {
      ...
      transformPasted: (slice) => {
        ...
        for (const rule of regexPasteRules2) {
          ...
          const match = (0, import_core_helpers.findMatches)(textContent, rule.regexp)[0];
          if (canBeReplaced && match && rule.type === "mark" && rule.replaceSelection) {
            ...
            if (typeof rule.replaceSelection !== "boolean" ? rule.replaceSelection(textContent2) : rule.replaceSelection) {
              ...
              const attributes = (0, import_core_helpers.isFunction)(getAttributes) ? getAttributes(match, true) : getAttributes;
              ...
              return import_prosemirror_model.Slice.maxOpen(import_prosemirror_model.Fragment.fromArray(newTextNodes));
            }
          }
          ...
          if (transformed) {
            slice = rule.type === "node" && rule.nodeType.isBlock ? new import_prosemirror_model.Slice(import_prosemirror_model.Fragment.fromArray(transformedNodes), 0, 0) : new import_prosemirror_model.Slice(import_prosemirror_model.Fragment.fromArray(transformedNodes), slice.openStart, slice.openEnd);
          }
        }
...
var pastePluginKey = new import_prosemirror_state.PluginKey("pasteRule");
  ...
  return function handler(props) {
    ...
    fragment.forEach((child) => {
      ...
      if (!child.isText) {
        ...
        const content = import_prosemirror_model.Fragment.fromArray(childResult.nodes);
      ...
      let pos = 0;
      for (const match of (0, import_core_helpers.findMatches)(text, regexp)) {
...
function markRuleTransformer(props) {
  ...
  const attributes = (0, import_core_helpers.isFunction)(getAttributes) ? getAttributes(match, false) : getAttributes;
...
function isRegexPastRule(rule) {
  return (0, import_core_helpers.includes)(regexPasteRules, rule.type);
}
...
function resolvedPosInCode($pos) {
  for (const depth of (0, import_core_helpers.range)($pos.depth, 1)) {
...
function fixSliceOpening(slice) {
  const max = import_prosemirror_model.Slice.maxOpen(slice.content);
