...
__export(src_exports, {
  ...
  createCenteredAnnotationPositioner: () => createCenteredAnnotationPositioner
});
...
function toSegments(annotations) {
  ...
  const sortedPositions = (0, import_core.sort)([...positionMap.entries()], ([a], [z]) => a - z);
...
var AnnotationState = class {
  constructor(getStyle, store) {
     ...
     */
    __publicField(this, "decorationSet", import_view.DecorationSet.empty);
  }
  ...
  createDecorations(tr, annotations = []) {
    const decos = toSegments(annotations).map((segment) => {
      ...
      return import_view.Decoration.inline(segment.from, segment.to, {
        class: classNames.length > 0 ? classNames.join(" ") : void 0,
        style
      });
    });
    return import_view.DecorationSet.create(tr.doc, decos);
  }
  apply({ tr, action }) {
...
_AnnotationExtension_decorators = [(0, import_core2.extension)({
  defaultOptions: {
  ...
  defaultPriority: import_core2.ExtensionPriority.Low
})];
var _AnnotationExtension = class _AnnotationExtension extends (_a = import_core2.PlainExtension, _addAnnotation_dec = [(0, import_core2.command)()], _updateAnnotation_dec = [(0, import_core2.command)()], _removeAnnotations_dec = [(0, import_core2.command)()], _setAnnotations_dec = [(0, import_core2.command)()], _redrawAnnotations_dec = [(0, import_core2.command)()], _getAnnotations_dec = [(0, import_core2.helper)()], _getAnnotationsAt_dec = [(0, import_core2.helper)()], _selectionHasAnnotation_dec = [(0, import_core2.helper)()], _a) {
  constructor() {
    super(...arguments);
  ...
  onSetOptions(props) {
    ...
    if (!(0, import_core2.isEmptyObject)(changedPluginOptions)) {
  ...
  createPlugin() {
    ...
    if (this.options.getMap) {
      (0, import_core2.assert)(
        import_core2.environment.isProduction || !this.options.getStore,
  ...
  getAnnotationsAt(pos, includeEdges = true) {
    ...
    const state = this.getPluginState();
    const { from, to } = (0, import_core2.getTextSelection)(pos != null ? pos : selection, doc);
    for (const annotation of state.annotations) {
      if ((0, import_core2.within)(from, annotation.from, annotation.to) || (0, import_core2.within)(to, annotation.from, annotation.to) || (0, import_core2.within)(annotation.from, from, to) || (0, import_core2.within)(annotation.to, from, to)) {
        if (includeEdges) {
...
function createCenteredAnnotationPositioner(getAnnotationsAt) {
  return import_extension_positioner.selectionPositioner.clone({
    getActive: (props) => {
