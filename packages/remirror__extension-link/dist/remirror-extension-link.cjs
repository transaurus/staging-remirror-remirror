...
__export(src_exports, {
  ...
  TOP_50_TLDS: () => TOP_50_TLDS,
  extractHref: () => extractHref
});
module.exports = __toCommonJS(src_exports);

// src/link-extension.ts
var import_extract_domain = __toESM(require("extract-domain"), 1);
...
_LinkExtension_decorators = [(0, import_core.extension)({
  defaultOptions: {
    autoLink: false,
    defaultProtocol: "",
  ...
  defaultPriority: import_core.ExtensionPriority.Medium
})];
var LinkExtension = class extends (_c = import_core.MarkExtension, _shortcut_dec = [(0, import_core.keyBinding)({ shortcut: import_core.NamedShortcut.InsertLink })], _updateLink_dec = [(0, import_core.command)()], _selectLink_dec = [(0, import_core.command)()], _removeLink_dec = [(0, import_core.command)()], _c) {
  constructor() {
    super(...arguments);
  ...
  createTags() {
    return [import_core.ExtensionTag.Link, import_core.ExtensionTag.ExcludeInputRules];
  }
  createMarkSpec(extra, override) {
    ...
    const getTargetObject = (target) => {
      ...
      return target && (0, import_core.includes)(targets, target) ? { target } : void 0;
    };
    return __spreadProps(__spreadValues({
      ...
      toDOM: (node) => {
        const _a2 = (0, import_core.omitExtraAttributes)(node.attrs, extra), { auto: _, target: __ } = _a2, rest = __objRest(_a2, ["auto", "target"]);
  ...
  shortcut({ tr }) {
    ...
    const mark = (0, import_core.getMarkRange)($from, this.type);
    if (empty) {
      const selectedWord = mark != null ? mark : (0, import_core.getSelectedWord)(tr);
      if (!selectedWord) {
        return false;
      }
  ...
  updateLink(attrs, range) {
    return (props) => {
      const { tr } = props;
      const selectionIsValid = (0, import_core.isTextSelection)(tr.selection) && !(0, import_core.isSelectionEmpty)(tr.selection) || (0, import_core.isAllSelection)(tr.selection) || (0, import_core.isMarkActive)({ trState: tr, type: this.type });
      if (!selectionIsValid && !range) {
        return false;
      }
      tr.setMeta(this.name, { command: UPDATE_LINK, attrs, range });
      return (0, import_core.updateMark)({ type: this.type, attrs, range })(props);
    };
  }
  selectLink() {
  ...
  removeLink(range) {
    return (props) => {
      const { tr } = props;
      if (!(0, import_core.isMarkActive)(__spreadValues({ trState: tr, type: this.type }, range))) {
        return false;
      }
      return (0, import_core.removeMark)({ type: this.type, expand: true, range })(props);
    };
  }
  /**
    ...
    return [
      {
        ...
        getAttributes: (url, isReplacement) => ({
          href: this.buildHref((0, import_core.getMatchString)(url)),
          auto: !isReplacement
        }),
        transformMatch: (match) => {
          const url = (0, import_core.getMatchString)(match);
          if (!url) {
            return false;
          }
  ...
  createPlugin() {
    return {
      appendTransaction: (transactions, prevState, state) => {
        ...
        const isUndo = (0, import_history.undoDepth)(prevState) - (0, import_history.undoDepth)(state) === 1;
        if (isUndo) {
          return;
        }
        ...
        const composedTransaction = (0, import_core.composeTransactionSteps)(transactions, prevState);
        const changes = (0, import_core.getChangedRanges)(composedTransaction, [import_transform.ReplaceAroundStep, import_transform.ReplaceStep]);
  ...
  findAutoLinks(str) {
    ...
    const toAutoLink = [];
    for (const match of (0, import_core.findMatches)(str, this.options.autoLinkRegex)) {
      const text = (0, import_core.getMatchString)(match);
      if (!text) {
        continue;
      }
  ...
  isValidTLD(str) {
    ...
    const domain = (0, import_extract_domain.default)(str);
    if (domain === "") {
      return true;
    }
    const tld = (0, import_core.last)(domain.split("."));
