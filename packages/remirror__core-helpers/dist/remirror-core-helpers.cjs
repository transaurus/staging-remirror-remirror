...
function cleanupOS(os, pattern, label) {
  ...
  return format(
    (_a = os.replace(/ ce$/i, " CE").replace(/\bhpw/i, "web").replace(/\bMacintosh\b/, "Mac OS").replace(/_powerpc\b/i, " OS").replace(/\b(os x) [^\d ]+/i, "$1").replace(/\bMac (OS X)\b/, "$1").replace(/\/(\d)/, " $1").replace(/_/g, ".").replace(/(?: bepc|[ .]*fc[\d .]+)$/i, "").replace(/\bx86\.64\b/gi, "x86_64").replace(/\b(Windows Phone) OS\b/, "$1").replace(/\b(Chrome OS \w+) [\d.]+\b/, "$1").split(" on ")[0]) != null ? _a : ""
  );
}
function isAndroidOS() {
  ...
  return cleanupOS((_a = match[0]) != null ? _a : "", "Android", "Android").includes("Android");
}
...
function unset(path, target) {
  ...
  for (const [index, key] of path.entries()) {
    ...
    item = isArray(item) ? [...item] : __spreadValues({}, item);
    value[key] = item;
    value = item;
  }
  return clonedObject;
...
function cx(...classes) {
  return uniqueArray((0, import_clsx.clsx)(...classes).split(" ")).join(" ");
}
...
function createErrorMessage(code, extraMessage) {
  ...
  const prefix = message ? "".concat(message, "\n\n") : "";
  const customMessage = extraMessage ? "".concat(extraMessage, "\n\n") : "";
...
var RemirrorError = class _RemirrorError extends import_make_error2.BaseError {
  /**
...
function freeze(target, options = {}) {
  ...
  return new Proxy(target, {
    ...
    set: (_, prop) => {
      invariant(false, {
        message: "It seems you're trying to set the value of the property (".concat(String(
          prop
