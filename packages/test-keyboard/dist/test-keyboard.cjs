...
__export(src_exports, {
  ...
  getModifierInformation: () => getModifierInformation,
  isUSKeyboardCharacter: () => isUSKeyboardCharacter,
  noKeyPress: () => noKeyPress,
  noKeyUp: () => noKeyUp,
...
function isUSKeyboardCharacter(char) {
  return (0, import_core_helpers.isString)(char) && Object.keys(rawUSKeyboardLayout).includes(char);
}
...
function cleanKey(key) {
  return (0, import_core_helpers2.omit)(usKeyboardLayout[key], ["shiftKey"]);
}
...
var Keyboard = class _Keyboard {
   ...
   */
  usChar({ text, options = (0, import_core_helpers3.object)(), typing = false }) {
   ...
   */
  type({ text, options = (0, import_core_helpers3.object)() }) {
   ...
   * @param props - see {@link TextInputProps}
   */
  mod({ text, options = (0, import_core_helpers3.object)() }) {
    var _a;
    let modifiers = text.split(/-(?!$)/);
    let result = (_a = modifiers[modifiers.length - 1]) != null ? _a : "";
    modifiers = (0, import_core_helpers3.take)(modifiers, modifiers.length - 1);
    if (result === "Space") {
      result = " ";
    }
  ...
  fireAllEvents({ options, typing = false }) {
    ...
    if (!(0, import_core_helpers3.includes)(noKeyPress, options.key) || typing && isUSKeyboardCharacter(options.key) && usKeyboardLayout[options.key].text) {
      this.keyPress({ options });
    }
    if (!(0, import_core_helpers3.includes)(noKeyUp, options.key)) {
      this.keyUp({ options });
    }
    return this;
  }
