...
__export(src_exports, {
  NodeFormattingExtension: () => NodeFormattingExtension
});
...
var increaseIndentOptions = {
  label: ({ t }) => t(import_messages.ExtensionNodeFormattingMessages.INCREASE_INDENT_LABEL),
  icon: "indentIncrease"
};
var decreaseIndentOptions = {
  label: ({ t }) => t(import_messages.ExtensionNodeFormattingMessages.DECREASE_INDENT_LABEL),
  icon: "indentDecrease"
};
var centerAlignOptions = {
  label: ({ t }) => t(import_messages.ExtensionNodeFormattingMessages.CENTER_ALIGN_LABEL),
  icon: "alignCenter",
  active: createActiveCheck(({ node }) => node.attrs.nodeTextAlignment === "center")
};
var justifyAlignOptions = {
  label: ({ t }) => t(import_messages.ExtensionNodeFormattingMessages.JUSTIFY_ALIGN_LABEL),
  icon: "alignJustify",
  active: createActiveCheck(({ node }) => node.attrs.nodeTextAlignment === "justify")
};
var rightAlignOptions = {
  label: ({ t }) => t(import_messages.ExtensionNodeFormattingMessages.RIGHT_ALIGN_LABEL),
  icon: "alignRight",
  active: createActiveCheck(({ node }) => node.attrs.nodeTextAlignment === "right")
};
var leftAlignOptions = {
  label: ({ t }) => t(import_messages.ExtensionNodeFormattingMessages.LEFT_ALIGN_LABEL),
  icon: "alignLeft",
...
function extractIndent(indents, marginLeft) {
  var _a2;
  const largestIndent = (0, import_core.extractPixelSize)((0, import_core.last)(indents));
  const value = (0, import_core.extractPixelSize)(marginLeft != null ? marginLeft : "0");
  const max = (_a2 = indents.length) != null ? _a2 : 1;
  const indentIncrement = largestIndent / max;
  return (0, import_core.clamp)({ max, min: 0, value: Math.floor(value / indentIncrement) });
}
var NUMERIC_REGEX = /^\d+(?:\.\d+)?$/;
var PERCENT_REGEX = /^(\d+(?:\.\d+)?)%$/;
function extractLineHeight(lineHeight) {
  if ((0, import_core.isNumber)(lineHeight)) {
    return lineHeight;
  }
  if (!lineHeight) {
    return null;
...
_NodeFormattingExtension_decorators = [(0, import_core2.extension)({
  defaultOptions: {
    indents: [
      "0",
      "20px",
...
var NodeFormattingExtension = class extends (_a = import_core2.PlainExtension, _setLineHeight_dec = [(0, import_core2.command)()], _setTextAlignment_dec = [(0, import_core2.command)()], _setIndent_dec = [(0, import_core2.command)()], _centerAlign_dec = [(0, import_core2.command)(centerAlignOptions)], _justifyAlign_dec = [(0, import_core2.command)(justifyAlignOptions)], _leftAlign_dec = [(0, import_core2.command)(leftAlignOptions)], _rightAlign_dec = [(0, import_core2.command)(rightAlignOptions)], _increaseIndent_dec = [(0, import_core2.command)(increaseIndentOptions)], _decreaseIndent_dec = [(0, import_core2.command)(decreaseIndentOptions)], _centerAlignShortcut_dec = [(0, import_core2.keyBinding)({ shortcut: import_core2.NamedShortcut.CenterAlignment, command: "centerAlign" })], _justifyAlignShortcut_dec = [(0, import_core2.keyBinding)({ shortcut: import_core2.NamedShortcut.JustifyAlignment, command: "justifyAlign" })], _leftAlignShortcut_dec = [(0, import_core2.keyBinding)({ shortcut: import_core2.NamedShortcut.LeftAlignment, command: "leftAlign" })], _rightAlignShortcut_dec = [(0, import_core2.keyBinding)({ shortcut: import_core2.NamedShortcut.RightAlignment, command: "rightAlign" })], _increaseIndentShortcut_dec = [(0, import_core2.keyBinding)({
  shortcut: import_core2.NamedShortcut.IncreaseIndent,
...
})], _decreaseIndentShortcut_dec = [(0, import_core2.keyBinding)({
  shortcut: import_core2.NamedShortcut.DecreaseIndent,
  command: "decreaseIndent",
  // Ensure this has higher priority than the dedent keybinding in @remirror/extension-list
  priority: import_core2.ExtensionPriority.Medium
})], _a) {
  constructor() {
    super(...arguments);
    ...
    return [
      {
        identifiers: {
          type: "node",
          tags: [import_core2.ExtensionTag.FormattingNode],
          ...
          style: {
            ...
            toDOM: ({ nodeIndent, nodeTextAlignment, nodeLineHeight, style }) => {
              ...
              return {
                ...
                style: (0, import_core2.joinStyles)({ marginLeft, textAlign, lineHeight }, style)
              };
            }
          }
        }
  ...
  setIndent(level) {
    return this.setNodeAttribute(({ node }) => {
      ...
      const indent = (0, import_core2.clamp)({ min: 0, max: this.options.indents.length - 1, value });
  ...
  setNodeAttribute(getAttributes) {
    return (props) => {
      ...
      if ((0, import_core2.isEmptyArray)(gatheredNodes)) {
        return false;
      }
      if (!dispatch) {
        return true;
      ...
      if ((0, import_core2.isEmptyArray)(updates)) {
        return false;
      }
      if (!dispatch) {
        return true;
