...
__export(src_exports, {
  RemirrorTestChain: () => RemirrorTestChain,
  TagTracker: () => TagTracker,
  extensionValidityTest: () => extensionValidityTest,
  ignoreJSDOMWarnings: () => ignoreJSDOMWarnings,
  jsdomPolyfill: () => jsdomPolyfill,
  remirrorMatchers: () => remirrorMatchers,
  renderEditor: () => renderEditor,
  setupRemirrorEnvironment: () => setupRemirrorEnvironment
});
module.exports = __toCommonJS(src_exports);

// src/jest-remirror-editor.ts
var import_dom = require("@testing-library/dom");
var import_jest_prosemirror = require("jest-prosemirror");
var import_core3 = require("@remirror/core");
var import_dom2 = require("@remirror/dom");
...
function text(value, schema) {
  const matches = (0, import_core.findMatches)(value, /(\\+)?<(\w+)>/g);
  const tags = (0, import_core.object)();
  let stripped = "";
  let textIndex = 0;
...
function offsetTags(tags, offset) {
  const result = (0, import_core.object)();
...
function isTaggedProsemirrorNode(value) {
  return (0, import_core.isProsemirrorNode)(value) && !isTagTracker(value);
}
function sequence(...content) {
  let position = 0;
  let tags = (0, import_core.object)();
  const nodes = [];
...
function coerce(props) {
  ...
  const taggedContent = content.map(
    (item) => (0, import_core.isString)(item) ? text(item, schema) : item
  );
  return sequence(...(0, import_core.flattenArray)(taggedContent));
}
function nodeFactory(props) {
  const { name, schema, attrs, marks } = props;
  const nodeBuilder = (0, import_core.hasOwnProperty)(schema.nodes, name) ? schema.nodes[name] : void 0;
  (0, import_core.invariant)(nodeBuilder, {
    message: 'Node: "'.concat(name, "\" doesn't exist in the schema. The current schema contains the following nodes: ").concat((0, import_core.keys)(
      schema.nodes
    ).join(", "))
  });
  return (...content) => {
...
function markFactory(props) {
  ...
  const markBuilder = (0, import_core.hasOwnProperty)(schema.marks, name) ? schema.marks[name] : void 0;
  (0, import_core.invariant)(markBuilder, {
    message: 'Mark: "'.concat(name, "\" doesn't exist in the schema. The current schema contains the following marks: ").concat((0, import_core.keys)(
      schema.marks
    ).join(", "))
  });
  return (...content) => {
...
function renderEditor(extensions, _a = (0, import_core3.object)()) {
  ...
  const manager = (0, import_dom2.createDomManager)(extensions, options);
  (0, import_dom2.createDomEditor)(__spreadProps(__spreadValues({}, props), { element, manager }));
...
var _RemirrorTestChain = class _RemirrorTestChain {
  constructor(manager) {
     ...
     */
    __publicField(this, "nodes", (0, import_core3.object)());
    /**
     * The marks available for building up the prosemirror document.
     */
    __publicField(this, "marks", (0, import_core3.object)());
    /**
     ...
     */
    __publicField(this, "attributeNodes", (0, import_core3.object)());
    /**
     ...
     */
    __publicField(this, "attributeMarks", (0, import_core3.object)());
    /**
    ...
    __publicField(this, "add", (taggedDocument) => {
      ...
      (0, import_core3.invariant)(taggedDocument.type === schema.topNodeType, {
    ...
    __publicField(this, "selectText", (selection) => {
      const tr = this.tr;
      const textSelection = (0, import_core3.getTextSelection)(selection, tr.doc);
    ...
    __publicField(this, "callback", (fn) => {
      fn((0, import_core3.pick)(this, ["helpers", "commands", "to", "state", "tags", "from", "doc", "view"]));
      return this;
    });
    /**
    ...
    __publicField(this, "shortcut", (text2) => {
      (0, import_jest_prosemirror.shortcut)({ shortcut: text2, view: this.view });
      return this;
    });
    /**
    ...
    __publicField(this, "paste", (content) => {
      (0, import_jest_prosemirror.pasteContent)({ view: this.view, content });
      return this;
    });
    /**
    ...
    __publicField(this, "press", (char, times = 1) => {
      for (const _ of (0, import_core3.range)(times)) {
        (0, import_jest_prosemirror.press)({ char, view: this.view });
      }
      return this;
    });
    /**
    ...
    __publicField(this, "backspace", (times) => {
      (0, import_jest_prosemirror.backspace)({ view: this.view, times });
      return this;
    });
    /**
    ...
    __publicField(this, "forwardDelete", (times) => {
      (0, import_jest_prosemirror.forwardDelete)({ view: this.view, times });
      return this;
    });
    /**
    ...
    __publicField(this, "fire", (parameters) => {
      (0, import_jest_prosemirror.fireEventAtPosition)(__spreadValues({ view: this.view }, parameters));
      return this;
    });
    /**
    ...
    __publicField(this, "jumpTo", (pos, endPos) => {
      if (pos === "start") {
        (0, import_jest_prosemirror.dispatchTextSelection)({ view: this.view, start: 1 });
        return this;
      }
      if (pos === "end") {
        (0, import_jest_prosemirror.dispatchTextSelection)({ view: this.view, start: this.doc.content.size - 1 });
        return this;
      }
      (0, import_jest_prosemirror.dispatchTextSelection)({ view: this.view, start: pos, end: endPos });
      return this;
    });
    /**
    ...
    __publicField(this, "insertText", (text2) => {
      ...
      (0, import_jest_prosemirror.insertText)({ start: from, text: text2, view: this.view });
      return this;
    });
    /**
    ...
    __publicField(this, "debug", (element = this.view.dom) => {
      console.log((0, import_dom.prettyDOM)(element));
      return this;
    });
    /**
   ...
   */
  get copied() {
    return (0, import_jest_prosemirror.copyContent)({ view: this.view });
  }
  /**
  ...
  createDocBuilders() {
    ...
    for (const extension of __privateGet(this, _manager).extensions) {
      if ((0, import_core3.isMarkExtension)(extension)) {
        this.marks[extension.name] = markFactory({
          name: extension.name,
          schema: this.schema
        });
        this.attributeMarks[extension.name] = (attrs = (0, import_core3.object)()) => markFactory({ name: extension.name, schema: this.schema, attrs });
      }
      if ((0, import_core3.isNodeExtension)(extension)) {
        this.nodes[extension.name] = nodeFactory({
          name: extension.name,
          schema: this.schema
        });
        this.attributeNodes[extension.name] = (attrs = (0, import_core3.object)()) => nodeFactory({ name: extension.name, schema: this.schema, attrs });
      }
    }
  }
};
...
if ((0, import_core3.isFunction)(afterEach)) {
  afterEach(() => {
    cleanup();
  });
}
...
var remirrorMatchers = {
  toContainRemirrorDocument(state, value) {
    var _a;
    if (!(0, import_core4.isProsemirrorNode)(value)) {
      return {
        pass: false,
        message: () => "The expected value should be an instance of ProsemirrorNode."
      };
    }
    if (!(0, import_core4.isEditorState)(state)) {
      return {
        pass: false,
  ...
  toEqualRemirrorDocument(doc, value) {
    if (!(0, import_core4.isProsemirrorNode)(value) || !(0, import_core4.isProsemirrorNode)(doc)) {
      return {
        pass: false,
...
function jsdomPolyfill() {
  if (!import_core5.environment.isJSDOM) {
    return;
  }
...
function supportBoundingClientRect() {
  ...
  Element.prototype.getBoundingClientRect = function() {
    ...
    if ((0, import_core5.isFunction)(originalGetBoundingClientRect)) {
      try {
  ...
  Element.prototype.getClientRects = function() {
    if ((0, import_core5.isFunction)(originalGetClientRects)) {
      try {
...
function supportCancelAnimationFrame() {
  if ((0, import_core5.isFunction)(window.cancelAnimationFrame)) {
    return;
  }
...
function supportRanges() {
  ...
  function fakeCreateRange() {
    return {
      setStart: import_core5.noop,
      setEnd: import_core5.noop,
...
function setupRemirrorEnvironment() {
  if (!import_core6.environment.isBrowser) {
    return;
  }
...
function extensionValidityTest(Extension, ...[options]) {
  describe("`".concat(Extension.name, "`"), () => {
    ...
    it("can safely be cloned", () => {
      ...
      expect((0, import_core7.pick)(clonedExtension.options, dynamicKeys)).toEqual(
        (0, import_core7.pick)(extension2.options, dynamicKeys)
      );
    });
    let defaultOptions = (0, import_core7.object)();
    (0, import_core7.mutateDefaultExtensionOptions)((value) => {
      defaultOptions = value;
    });
    const handlerKeys = Extension.handlerKeys.map((key) => [key]);
    if (!(0, import_core7.isEmptyArray)(handlerKeys)) {
      it.each(handlerKeys)("has valid handlerKey: %s", (handlerKey) => {
        const extension2 = new Extension(options);
        expect((0, import_core7.isFunction)(extension2.options[handlerKey])).toBe(true);
      });
    }
    it("has the correct options", () => {
      const extension2 = new Extension(options);
      const expectedOptions = __spreadValues(__spreadValues(__spreadValues({}, defaultOptions), Extension.defaultOptions), options);
      expect((0, import_core7.omit)(extension2.options, Extension.handlerKeys)).toEqual(expectedOptions);
    });
    const extension = new Extension(options);
    if ((0, import_core7.isNodeExtension)(extension)) {
      it("creates a valid node spec", async () => {
        ...
        expect((0, import_core7.isPlainObject)(spec)).toBe(true);
        renderEditor([extension], {
          props: { initialContent: "" },
          stringHandler: "html"
        });
        expect((0, import_core7.isNodeType)(extension.type)).toBe(true);
    ...
    if ((0, import_core7.isMarkExtension)(extension)) {
      it("creates a valid mark spec", async () => {
        ...
        expect((0, import_core7.isMarkType)(extension.type)).toBe(true);
        if (Extension.disableExtraAttributes) {
          return;
        }
        expect(extraAttributes.defaults.getCalled()).toBe(true);
