...
var import_string_prototype = __toESM(require("string.prototype.matchall"), 1);
...
_FindExtension_decorators = [(0, import_core.extension)({
  defaultOptions: {
...
var FindExtension = class extends (_a = import_core.PlainExtension, _find_dec = [(0, import_core.command)()], _stopFind_dec = [(0, import_core.command)()], _findAndReplace_dec = [(0, import_core.command)()], _findAndReplaceAll_dec = [(0, import_core.command)()], _findRanges_dec = [(0, import_core.helper)()], _a) {
  constructor() {
    super(...arguments);
  ...
  setProps({
    ...
    this._query = (0, import_escape_string_regexp.default)(query);
  ...
  gatherFindResults(doc) {
    ...
    doc.descendants((node, pos) => {
      ...
      const start = pos + 1;
      for (const match of (0, import_string_prototype.default)(node.textContent, re)) {
  ...
  createDecorationSet(doc) {
    const decorations = this._ranges.map(
      (deco, index) => import_view.Decoration.inline(
        deco.from,
        deco.to,
